...
class TranslateController extends TranslateAppController {
    ...
    public function admin_index($id = null, $modelAlias = null) {
        ...
        if (!isset($record[getModelAlias($modelAlias)]['id'])) {
            $this->Session->setFlash(__('Invalid record.'), 'default', array('class' => 'error'));
            $this->redirect(array(
                'plugin' => null,
                'controller' => Inflector::pluralize(getModelAlias($modelAlias)),
                'action' => 'index',
            ));
        }
        $this->set('title_for_layout', sprintf(__('Translations: %s'), $record[getModelAlias($modelAlias)][$model->displayField]));

        $runtimeModel = & $model->translateModel();
        $runtimeModelAlias = $runtimeModel->alias;
        $translations = $runtimeModel->find('all', array(
            'conditions' => array(
                $runtimeModelAlias . '.model' => getModelAlias($modelAlias),
                $runtimeModelAlias . '.foreign_key' => $id,
                $runtimeModelAlias . '.field' => $this->getTitleField($modelAlias),
            ),
                ));
    ...
    private function getTitleField($modelAlias = null) {
        if ($modelAlias == null) {
            throw new Exception('modelAlias cannot be null');
        }
        $config = Configure::read('Translate.models');
        if (isset($config[$modelAlias])) {
            foreach ($config[$modelAlias] as $key => $val) {
                if ($val == 'titleTranslation')
                    return $key;
            }
        }
        return 'title';
