 ...
 */
	public function startup(Controller $controller) {
		$this->__blogURL = Configure::read('Service.akismet_url');
		$this->__wordPressAPIKey = Configure::read('Service.akismet_key');

		// Set some default values
		$this->__apiPort = 80;
		$this->__akismetServer = 'rest.akismet.com';
		$this->__akismetVersion = '1.1';

		// Start to populate the comment data
		$this->__comment['blog'] = $this->__blogURL;
		$this->__comment['user_agent'] = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : 'Unknown';

		if (isset($_SERVER['HTTP_REFERER'])) {
			$this->__comment['referrer'] = $_SERVER['HTTP_REFERER'];
		}
		 ...
		 * PHP5 one...
		 */
		$this->__comment['user_ip'] = $_SERVER['REMOTE_ADDR'] != getenv('SERVER_ADDR') ? $_SERVER['REMOTE_ADDR'] : getenv('HTTP_X_FORWARDED_FOR');
	}
 ...
 */
	public function isKeyValid() {
		...
		$response = $this->__sendRequest('key=' . $this->__wordPressAPIKey . '&blog=' . $this->__blogURL, $this->__akismetServer, '/' . $this->__akismetVersion . '/verify-key');
 ...
 *
 */
	private function __sendRequest($request, $host, $path) {
		...
		$httpRequest .= "User-Agent: Akismet PHP5 Class " . $this->__version . " | Akismet/1.11\r\n";
		$httpRequest .= "\r\n";
		$httpRequest .= $request;

		$socketWriteRead = new SocketWriteRead($host, $this->__apiPort, $httpRequest);
 ...
 */
	private function __getQueryString() {
		foreach ($_SERVER as $key => $value) {
			if (!in_array($key, $this->__ignore)) {
				if ($key == 'REMOTE_ADDR') {
					$this->__comment[$key] = $this->__comment['user_ip'];
				} else {
					$this->__comment[$key] = $value;
				}
			}
		}
		...
		foreach ($this->__comment as $key => $data) {
			if (!is_array($data)) {
 ...
 */
	public function isCommentSpam() {
		$response = $this->__sendRequest($this->__getQueryString(), $this->__wordPressAPIKey . '.rest.akismet.com', '/' . $this->__akismetVersion . '/comment-check');
 ...
 */
	public function submitSpam() {
		$this->__sendRequest($this->__getQueryString(), $this->__wordPressAPIKey . '.' . $this->__akismetServer, '/' . $this->__akismetVersion . '/submit-spam');
	}
 ...
 */
	public function submitHam() {
		$this->__sendRequest($this->__getQueryString(), $this->__wordPressAPIKey . '.' . $this->__akismetServer, '/' . $this->__akismetVersion . '/submit-ham');
	}
 ...
 */
	public function setReferrer($referrer) {
		$this->__comment['referrer'] = $referrer;
	}
 ...
 */
	public function setPermalink($permalink) {
		$this->__comment['permalink'] = $permalink;
	}
 ...
 */
	public function setCommentType($commentType) {
		$this->__comment['comment_type'] = $commentType;
	}
 ...
 */
	public function setCommentAuthor($commentAuthor) {
		$this->__comment['comment_author'] = $commentAuthor;
	}
 ...
 */
	public function setCommentAuthorEmail($authorEmail) {
		$this->__comment['comment_author_email'] = $authorEmail;
	}
 ...
 */
	public function setCommentAuthorURL($authorURL) {
		$this->__comment['comment_author_url'] = $authorURL;
	}
 ...
 */
	public function setCommentContent($commentBody) {
		$this->__comment['comment_content'] = $commentBody;
	}
 ...
 */
	public function setAkismetServer($akismetServer) {
		$this->__akismetServer = $akismetServer;
	}
 ...
 */
	public function setAkismetVersion($akismetVersion) {
		$this->__akismetVersion = $akismetVersion;
	}
...
class SocketWriteRead {
	...
	private $__responseLength;

	private $__errorNumber;

	private $__errorString;
 ...
 */
	public function __construct($host, $port, $request, $responseLength = 1160) {
		...
		$this->__responseLength = $responseLength;
		$this->__errorNumber = 0;
		$this->__errorString = '';
	}
 ...
 */
	public function send() {
		...
		$fs = fsockopen($this->__host, $this->__port, $this->__errorNumber, $this->__errorString, 3);

		if ($this->__errorNumber != 0) {
			throw new Exception('Error connecting to host: ' . $this->__host . ' Error number: ' . $this->__errorNumber . ' Error message: ' . $this->__errorString);
		}

		if ($fs !== false) {
			@fwrite($fs, $this->__request);

			while (!feof($fs)) {
				$this->__response .= fgets($fs, $this->__responseLength);
			}
