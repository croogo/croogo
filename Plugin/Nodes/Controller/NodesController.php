 ...
 */
	public function index() {
		...
		$this->paginate['Node']['limit'] = $limit;
	...
	public function view($id = null) {
		if (isset($this->request->params['named']['slug']) && isset($this->request->params['named']['type'])) {
			...
			$node = $this->Node->find('first', array(
				...
				'cache' => array(
					'name' => 'node_' . $this->Croogo->roleId . '_' . $this->request->params['named']['type'] . '_' . $this->request->params['named']['slug'],
					'config' => 'nodes_view',
				),
			));
		} elseif ($id == null) {
	...
	protected function _viewFallback($views) {
		...
		foreach ($views as $view) {
			foreach ($viewPaths as $viewPath) {
				if ($this->theme) {
                                        $viewPath = $viewPath . 'Themed' . DS . $this->theme . DS .  'Plugin' . DS . $this->plugin . DS . $this->name . DS . $view . $this->ext;
