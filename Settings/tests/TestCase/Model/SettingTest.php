...
class SettingTest extends CroogoTestCase
{
    ...
    {
        $this->Setting->write('Site.title', 'My new site title', ['editable' => 1]);
        $siteTitle = $this->Setting->findByKey('Site.title');
        $this->assertEquals('My new site title', $siteTitle['Setting']['value']);

        $this->Setting->write('Site.title', 'My new site title', ['input_type' => 'checkbox']);
        $siteTitle = $this->Setting->findByKey('Site.title');
        $this->assertTrue($siteTitle['Setting']['editable']);

        $this->Setting->write('Site.title', 'My new site title', ['input_type' => 'textarea', 'editable' => false]);
        $siteTitle = $this->Setting->findByKey('Site.title');
