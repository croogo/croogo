...
<?php
    ...
    if ($type['Type']['format_show_author']) {
        ...
        if ($layout->node('User.website') != null) {
            $author = $this->Html->link($layout->node('User.name'), $layout->node('User.website'));
        } else {
    ...
    if ($type['Type']['format_show_date']) {
        echo ' ' . __('on', true) . ' ';
        echo $this->Html->tag('span', $time->format(Configure::read('Reading.date_time_format'), $layout->node('created'), null, Configure::read('Site.timezone')), array('class' => 'date'));
    }
?>
</div>